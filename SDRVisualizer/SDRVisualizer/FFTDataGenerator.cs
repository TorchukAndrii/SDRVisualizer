    ...
    private void GenerateData(bool showBorders = false, bool addSpikes = false)
    {
        ...
        for (var r = 0; r < rows; r++)
        {
            ...
            for (var c = 0; c < cols; c++)
            {
                ...
                var frequency = c / (double)cols; // Frequency is proportional to column index
                var sineAmplitude = Math.Sin(2 * Math.PI * frequency * r / 100); // Vary amplitude over time (rows)

                // Map the sine amplitude from [-1, 1] to [-120, -20]
                var amplitude = sineAmplitude * 50 - 70; // Maps sine to range [-120, -20]

                // Add some Gaussian noise to simulate FFT data

                var noise = _rand.NextDouble() * noiseLevel -
                            noiseLevel / 2; // Random noise between -noiseLevel/2 to +noiseLevel/2
